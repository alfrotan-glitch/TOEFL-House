...
export default function ProgramVersionsPanel({ branchId }: { branchId?: string } = {}) {
  ...
  const [placementConfig, setPlacementConfig] = useState<any>({ version: null, enabled: false, required: false, requirementMode: 'not_required', firstLevelExempt: false, expiresMinutes: null, decisionRules: [], components: [{ key:'skill_scores', type:'skill_scores', label:'Skills Assessment', required:true, weight:100, maxScore:100, durationMinutes:30, skills:['grammar','vocabulary','reading','listening','writing','speaking'], instructions:'Score each skill using the examiner rubric.' }], scoringModel:'weighted_average', allowRetake:true, maxAttempts:null, firstAttemptBillable:true, retakeBillable:false, retakeFeeAmount:null, passScore:60, instructions:'' });
  ...
  const loadTree = useCallback(async (id: string) => {
    ...
    try {
      ...
      try { const pp = await api.get<any>(`/academic/program-versions/${id}/placement-profile`); setPlacementProfile(pp); setPlacementConfig({ version: pp.version ?? null, enabled: pp.requirementMode !== 'not_required', required: pp.requirementMode === 'required', requirementMode: pp.requirementMode || 'not_required', firstLevelExempt: !!pp.firstLevelExempt, expiresMinutes: pp.expiresMinutes ?? null, decisionRules: pp.decisionRules || [], method: pp.method || 'skill_scores', components: pp.components || [], scoringModel: pp.scoringModel || 'weighted_average', allowRetake: pp.allowRetake !== false, maxAttempts: pp.maxAttempts ?? null, firstAttemptBillable: pp.firstAttemptBillable !== false, retakeBillable: pp.retakeBillable === true, retakeFeeAmount: pp.retakeFeeAmount ?? null, passScore: pp.passScore ?? 60, instructions: pp.instructions || '' }); } catch { setPlacementProfile(null); }
    } catch (err: unknown) {
              ...
              <div className="space-y-6">
                ...
                <div className="bg-indigo-50/30 border border-indigo-100 rounded-xl p-5">
                  <div className="flex items-center justify-between gap-4 mb-4">
                    ...
                    <span className={`rounded-full px-3 py-1 text-[10px] font-black ${placementConfig.requirementMode === 'not_required' ? 'bg-slate-100 text-slate-500' : 'bg-indigo-100 text-indigo-700'}`}>{placementConfig.requirementMode === 'not_required' ? 'Not required' : 'Assessment enabled'}</span>
                  </div>
                  <div className="grid grid-cols-1 @md:grid-cols-2 @4xl:grid-cols-3 gap-3 mb-5">
                    <div className="rounded-xl border border-slate-200 bg-white px-3 py-3 min-w-0"><label className={labelCls}>Placement requirement</label><select value={placementConfig.requirementMode} onChange={e=>setPlacementConfig((c:any)=>({...c,requirementMode:e.target.value, required: e.target.value==='required', enabled: e.target.value!=='not_required', firstLevelExempt: e.target.value==='required' ? c.firstLevelExempt : false}))} className={`${inputCls} min-w-0`}>
                      <option value="required">Required</option>
                      <option value="optional">Optional (may skip)</option>
                      <option value="not_required">Not required</option>
                    </select></div>
                    <label className="flex items-center gap-2 text-xs font-bold rounded-xl border border-slate-200 bg-white px-3 py-3 min-w-0 cursor-pointer"><input type="checkbox" disabled={placementConfig.requirementMode !== 'required'} checked={placementConfig.firstLevelExempt} onChange={e=>setPlacementConfig((c:any)=>({...c,firstLevelExempt:e.target.checked}))}/> <span>Exempt first level</span></label>
                    ...
                    <div className="rounded-xl border border-slate-200 bg-white px-3 py-3 min-w-0"><label className={labelCls}>Maximum attempts</label><input type="number" min="1" value={placementConfig.maxAttempts ?? ''} onChange={e=>setPlacementConfig((c:any)=>({...c,maxAttempts:e.target.value===''?null:Number(e.target.value)}))} className={inputCls} placeholder="Unlimited"/></div>
                    <label className="flex items-center gap-2 text-xs font-bold rounded-xl border border-slate-200 bg-white px-3 py-3 min-w-0"><input type="checkbox" checked={placementConfig.firstAttemptBillable} onChange={e=>setPlacementConfig((c:any)=>({...c,firstAttemptBillable:e.target.checked}))}/> <span>Bill first attempt</span></label>
                    <label className="flex items-center gap-2 text-xs font-bold rounded-xl border border-slate-200 bg-white px-3 py-3 min-w-0"><input type="checkbox" checked={placementConfig.retakeBillable} onChange={e=>setPlacementConfig((c:any)=>({...c,retakeBillable:e.target.checked}))}/> <span>Bill retakes</span></label>
                    <div className="rounded-xl border border-slate-200 bg-white px-3 py-3 min-w-0"><label className={labelCls}>Retake fee (AFN)</label><input type="number" min="0" step="1" disabled={!placementConfig.retakeBillable} value={placementConfig.retakeFeeAmount ?? ''} onChange={e=>setPlacementConfig((c:any)=>({...c,retakeFeeAmount:e.target.value===''?null:Number(e.target.value)}))} className={inputCls} placeholder="Branch fee"/></div>
                    ...
                    <div className="rounded-xl border border-indigo-200 bg-indigo-50/40 px-3 py-3 min-w-0">
                      ...
                      <div className="text-sm font-extrabold text-slate-800 mt-1 break-words">{placementConfig.method === 'hybrid' ? 'Hybrid' : placementConfig.method === 'skill_scores' ? 'Skill-based' : placementConfig.method === 'level_assessment' ? 'Level-based' : placementConfig.method === 'written_test' ? 'Written test' : placementConfig.method === 'content_test' ? 'Content test' : placementConfig.method === 'custom_score' ? 'Custom score' : 'Interview'}</div>
                    ...
                    {(placementConfig.components || []).map((c:any, index:number) => (
                      <div key={c.key} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                        <div className="grid grid-cols-1 @md:grid-cols-2 @3xl:grid-cols-4 gap-3 items-end">
                          ...
                          <div className="min-w-0"><label className={labelCls}>Type</label><select value={c.type} onChange={e=>updatePlacementComponent(index,{type:e.target.value,scoringMethod:e.target.value==='content_test'?'hybrid':'manual',testId:e.target.value==='content_test'?c.testId:undefined,skills:e.target.value==='skill_scores'?(c.skills||['grammar','vocabulary','reading','listening','writing','speaking']):undefined})} className={`${inputCls} min-w-0`}>{componentTypes.map(t=><option key={t.value} value={t.value}>{t.label}</option>)}</select></div>
                        ...
                        <div className="mt-3 grid grid-cols-1 @md:grid-cols-2 @3xl:grid-cols-4 gap-3 items-end">
                          ...
                          <div className="min-w-0"><label className={labelCls}>Scoring</label><select value={c.scoringMethod || (c.type==='content_test' ? 'hybrid' : 'manual')} onChange={e=>updatePlacementComponent(index,{scoringMethod:e.target.value})} className={`${inputCls} min-w-0`}>{c.type==='content_test' ? <><option value="auto">Auto</option><option value="manual">Manual</option><option value="hybrid">Hybrid</option></> : <option value="manual">Manual</option>}</select></div>
                        ...
                        {c.type === 'content_test' && (
                          <div className="mt-3 rounded-xl bg-indigo-50/50 border border-indigo-100 p-3">
                            ...
                            <select value={c.testId || ''} onChange={e=>{ const testId=e.target.value || undefined; const selected=(testBankTests || []).find((test:any)=>test.id===testId); const questionTypes=(selected?.questions || []).map((question:any)=>question.qtype); const hasAuto=questionTypes.some((type:string)=>type==='mcq'||type==='short_answer'); const hasManual=questionTypes.some((type:string)=>type==='essay'||type==='speaking'); updatePlacementComponent(index,{testId,scoringMethod:testId?(hasAuto&&hasManual?'hybrid':hasManual?'manual':'auto'):'hybrid'}); }} className={`${inputCls} min-w-0`}>
                  ...
                  <div className="flex justify-end mt-3"><button type="button" disabled={busy || !selectedId} onClick={()=>handleApiCall(async()=>{ const active=placementConfig.requirementMode!=='not_required'; await api.put(`/academic/program-versions/${selectedId}/placement-profile`, { ...placementConfig, enabled:active, required:placementConfig.requirementMode==='required', firstLevelExempt:placementConfig.requirementMode==='required'&&placementConfig.firstLevelExempt, components:active?(placementConfig.components||[]):[], decisionRules:active?(placementConfig.decisionRules||[]):[] }); }, 'Placement policy saved.')} className="px-4 py-2 rounded-xl bg-indigo-600 text-white text-xs font-bold">Save Assessment Blueprint</button></div>
                </div>
