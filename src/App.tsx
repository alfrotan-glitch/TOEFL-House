...
function AuthenticatedApp() {
  ...
  return (
    ...
    <div className="flex h-screen w-screen overflow-hidden font-sans select-none bg-ambient">
      {store.isTabLoading && !store.isLoading && (
        <div className="fixed top-20 end-4 z-50 flex items-center gap-2 rounded-full border border-slate-200 bg-white/95 px-3 py-2 text-xs font-semibold text-slate-600 shadow-lg backdrop-blur" role="status" aria-live="polite">
        ...
        <header className="h-16 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-4 md:px-8 shrink-0 z-10 shadow-sm sticky top-0">
          ...
          <div ref={notificationRef} className="flex items-center gap-3 relative">
            <button onClick={() => setShowGlobalSearch(true)} className="hidden md:flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-bold text-slate-500 hover:bg-white hover:border-indigo-200 hover:text-indigo-600" title="Global search">
              <Search className="w-4 h-4" /> <span>Search</span><span className="ms-1 rounded-md border border-slate-200 bg-white px-1.5 py-0.5 text-[9px] text-slate-400">Ctrl K</span>
            </button>
            ...
            <button onClick={() => setShowNotifications(!showNotifications)} className="p-2 bg-slate-50 border border-slate-200 rounded-xl hover:bg-slate-100 text-slate-500 hover:text-slate-800 cursor-pointer relative"><Bell className="w-4 h-4" />{unreadCount > 0 && <span className="absolute top-1 end-1.5 w-2 h-2 bg-rose-600 rounded-full ring-2 ring-white animate-pulse" />}</button>

            {showNotifications && (
              <div className="absolute top-12 start-0 w-80 bg-white border border-slate-200 rounded-2xl shadow-xl p-4 z-40 text-xs text-start space-y-3.5 animate-in fade-in duration-200">
      ...
      {toasts.length > 0 && (
        <div className="fixed bottom-6 start-6 z-50 flex flex-col gap-2">
          {toasts.map((t) => (
