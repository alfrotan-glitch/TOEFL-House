...
function info(string $m): void { echo "  \033[90m·\033[0m     $m\n"; }
function step(string $m): void { echo "\n\033[36m▶ $m\033[0m\n"; }
function finding(string $code, string $d): void { global $fail, $findings; $fail++; $findings[] = $code; echo "  \033[31mFINDING\033[0m [\033[31m$code\033[0m] $d\n"; }

final class Browser
{
    public string $xsrf = '';
    private string $jar;
    public function __construct(private string $base) { $this->jar = (string) tempnam('/tmp', 'e2e-'); @unlink($this->jar); }
    ...
    public function send(string $method, string $path, array $params = [], bool $json = false): array
    {
        ...
        $h = [];
        if ($method !== 'GET' && $this->xsrf !== '') { $h[] = 'X-XSRF-TOKEN: '.$this->xsrf; }
        if ($method !== 'GET') {
            if ($json) { $h[] = 'Content-Type: application/json'; $body = json_encode($params, JSON_THROW_ON_ERROR); }
            else { $body = http_build_query($params); }
        ...
        $raw = curl_exec($ch);
        if ($raw === false) { return ['status' => 0, 'location' => null, 'body' => curl_error($ch), 'json' => null]; }
        $hs = (int) curl_getinfo($ch, CURLINFO_HEADER_SIZE);
        $head = substr((string) $raw, 0, $hs); $respBody = substr((string) $raw, $hs);
        $status = (int) curl_getinfo($ch, CURLINFO_HTTP_CODE);
        $location = preg_match('/^Location:\s*(.+)$/mi', $head, $lm) ? trim($lm[1]) : null;
        if (preg_match_all('/Set-Cookie:\s*XSRF-TOKEN=([^;]+)/i', $head, $m)) { $this->xsrf = urldecode(end($m[1])); }
    ...
    public function get(string $p): array { return $this->send('GET', $p); }
    public function post(string $p, array $q = [], bool $j = false): array { return $this->send('POST', $p, $q, $j); }
    public function prime(string $p = '/login'): void { $this->get($p); }
}

$pdo = new PDO("pgsql:host=127.0.0.1;port=5432;dbname=$E2E_DB", 'postgres', 'postgres');
$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
function q(string $sql, array $p = []): ?array { global $pdo; $st = $pdo->prepare($sql); $st->execute($p); $r = $st->fetch(PDO::FETCH_ASSOC); return $r === false ? null : $r; }
function qv(string $sql, array $p = []): string { $r = q($sql, $p); return $r ? (string) array_values($r)[0] : ''; }
function qc(string $sql, array $p = []): int { global $pdo; $st = $pdo->prepare($sql); $st->execute($p); return (int) $st->fetchColumn(); }
...
function mustRedirect(Browser $b, string $label, string $path, array $params, ?string $expect = null): array
{
    ...
    if ($redir && $toOk) { ok("$label → {$r['status']}"); }
    else { finding("transport.$label", "HTTP {$r['status']} loc=".($r['location'] ?? '-').' body='.substr(strip_tags($r['body']), 0, 100)); }
    return $r;
}
...
curl_exec($ch); $csrfCode = (int) curl_getinfo($ch, CURLINFO_HTTP_CODE);
...
$owner = new Browser($BASE); $owner->prime();
...
$provision = function (string $fullName, string $username, string $password) use ($owner, $positionId): Browser {
    ...
    if ($state !== 'active') { finding('staff.activate', "$username assignment state=$state"); }
    $b = new Browser($GLOBALS['BASE']); $b->prime();
    $lr = $b->post('/login', ['username' => $username, 'password' => $password]);
    if (! in_array($lr['status'], [302, 303], true)) { finding('staff.login', "$username login → {$lr['status']}"); }
    return $b;
};
...
$nobody = new Browser($BASE); $nobody->prime();
...
foreach ($checks as [$label, $cond]) { $cond ? ok($label) : finding("verify.$label", $label.' failed'); }
...
if ($findings !== []) { echo 'Findings: '.implode(', ', array_unique($findings))."\n"; exit(1); }
