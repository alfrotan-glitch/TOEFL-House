...
export class ClassGenerationEngine {
  ...
  constructor(private db: Database.Database) {
    ...
    this.stmtGetVersion = db.prepare('SELECT * FROM program_versions WHERE id = ?');
    ...
    this.stmtGetTerm = db.prepare('SELECT start_date, end_date FROM academic_terms WHERE id = ?');
    ...
    for (const slot of slots.length ? slots : [null]) {
      ...
      for (const level of levels) {
        for (const gender of genderModes) {
          ...
          const fee = offering ? Number(offering.fee_snapshot || 0) : this.resolveFee(level, params.branchId);
          const capacity = room?.capacity ?? params.defaultCapacity ?? 20;
          if (offering && (!room || Number(room.capacity) <= 0)) throw new Error('Every generated class must use a physical room with a positive capacity.');
          ...
          items.push({
            ...
            minViableSize: level.min_viable_size || ACADEMIC_DEFAULTS.levelMinViableSize, fee, proposedName: name, genderPolicy: gender,
          });
        }
      }
    }
  ...
  publish(runId: string, actor?: string | null) {
    ...
    const levelIds = [...new Set(items.map(i => i.level_id).filter(Boolean))];
    const slotIds = [...new Set(items.map(i => i.time_slot_id).filter(Boolean))];
    
    const levelsMap = new Map<string, any>();
    if (levelIds.length > 0) {
      const placeholders = levelIds.map(() => '?').join(',');
      const levelRows = this.db.prepare(`SELECT * FROM levels WHERE id IN (${placeholders})`).all(...levelIds) as any[];
    ...
    if (slotIds.length > 0) {
      ...
      const slotRows = this.db.prepare(`SELECT * FROM time_slots WHERE id IN (${placeholders})`).all(...slotIds) as any[];
      slotRows.forEach(s => slotsMap.set(s.id, s));
    }

    const version = this.stmtGetVersion.get(run.program_version_id) as any;
    const runParams = run.params_json ? JSON.parse(run.params_json) : {};
    ...
    if (offeringId) {
      ...
      if (!offering || String(offering.branch_id) !== String(run.branch_id) || String(offering.program_version_id) !== String(run.program_version_id)) {
  ...
  private resolveLevels(params: GenerationParams) {
    if (params.levelIds?.length) {
      const rows = this.db
        .prepare(`SELECT * FROM levels WHERE id IN (SELECT value FROM json_each(?)) AND COALESCE(is_active,1)=1`)
        .all(JSON.stringify(params.levelIds)) as any[];
  ...
  private resolveSlots(branchId: string, slotIds?: string[]) {
    if (slotIds?.length) {
      const rows = this.db
        .prepare(`SELECT * FROM time_slots WHERE branch_id = ? AND id IN (SELECT value FROM json_each(?))`)
        .all(branchId, JSON.stringify(slotIds)) as any[];
