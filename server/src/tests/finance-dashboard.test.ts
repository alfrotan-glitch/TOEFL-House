...
beforeAll(async () => {
  ...
  db.prepare(`INSERT OR IGNORE INTO invoices (id, student_id, total_amount, discount_amount, net_amount, status, issue_date, due_date, branch_id, invoice_number, purpose)
    VALUES (?, 'stu_dash_1', 12000, 0, 12000, 'partial', ?, ?, ?, 'INV-DASH-1', 'other')`).run('dash_inv_open', date, date, BRANCH_A);
  ...
  db.prepare(`INSERT OR IGNORE INTO invoices (id, student_id, total_amount, discount_amount, net_amount, status, issue_date, due_date, branch_id, invoice_number, purpose)
    VALUES (?, 'stu_dash_2', 5000, 0, 5000, 'paid', ?, ?, ?, 'INV-DASH-2', 'other')`).run('dash_inv_paid', date, date, BRANCH_A);
