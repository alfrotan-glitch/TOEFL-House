...
academicRouter.get('/program-versions/:id/placement-profile', ah(async (req, res) => {
  ...
  res.json({ configured:true, programVersionId:version.id, programName:version.program_name, versionLabel:version.version_label, required:!!profile.required, enabled:!!profile.enabled, method:profile.method, sections, components, scoringModel:profile.scoring_model || 'weighted_average', allowRetake:!!profile.allow_retake, maxScore:Number(profile.max_score), passScore:Number(profile.pass_score), instructions:profile.instructions });
}));
