/**
 * ⚠️  MIRRORED FILE — DO NOT EDIT INDEPENDENTLY
 * ============================================================================
 * This is a byte-for-byte copy of `src/utils/jalali.ts` (frontend), below the
 * header. The server's tsconfig sets `rootDir: ./src`, so it cannot import
 * from the frontend tree; duplicating the module is the only way to share it
 * without restructuring the build.
 *
 * `system-integrity-guards.test.ts` asserts the two files stay identical, so
 * an edit to one without the other fails CI rather than drifting silently.
 * Edit `src/utils/jalali.ts`, then copy it here.
 */
