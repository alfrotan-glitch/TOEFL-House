...
/**
 * Financial-correction ledger authority.
 *
 * Recorded corrections are compensating Finance facts. An obligation
 * adjustment re-states the charge in the source period and an allocation
 * reversal that undoes financial aid must be reflected in the double-entry
 * ledger so the GL provably mirrors the money facts. An allocation reversal
 * that merely reclassifies the receivable carries no ledger entry.
 *
 * This extends the authoritative general ledger (000187) to corrections and
 * is one-way: removing the source type would let a correction exist without
...
return new class extends Migration
{
    public function up(): void
    {
        ...
        DB::statement("ALTER TABLE journals ADD CONSTRAINT journals_source_type_check CHECK (source_type IN ('obligation','payroll_result','payroll_liability','expense','payment','discount','refund','fund_allocation','correction','journal','other'))");
        ...
        DB::statement('DROP INDEX IF EXISTS journals_one_per_correction');
        DB::statement('CREATE UNIQUE INDEX journals_one_per_correction ON journals (source_id) WHERE source_type = \'correction\' AND source_id IS NOT NULL');
