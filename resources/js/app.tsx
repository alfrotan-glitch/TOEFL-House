    ...
    <>
      ...
      <main className="workspace" aria-labelledby="management-title">
        ...
        {management && <>
          ...
          <section className="panel" aria-labelledby="reports-heading"><div className="section-heading"><div><p className="eyebrow">Governed reporting</p><h2 id="reports-heading">Latest report runs</h2></div><span className="source-note">{management.latest_reports.length} visible run{management.latest_reports.length === 1 ? '' : 's'}</span></div><p className="muted">Incomplete or historic-unclassified report evidence remains auditable, but its numeric result is withheld from this decision-support view.</p>{management.latest_reports.length === 0 ? <p className="empty">No report run is available in this authorized scope.</p> : <ul className="report-list">{management.latest_reports.map((report) => <li key={report.id}><span><strong>{report.period_key}</strong><small>{report.created_at ?? 'Timestamp unavailable'}</small></span><span className="status-chip">{report.evidence_status === 'complete' ? humanize(report.result ?? '') : report.evidence_status === 'incomplete' ? 'Withheld — evidence incomplete' : 'Withheld — historic evidence unclassified'}</span></li>)}</ul>}</section>
        </>}
      </main>
    </>
  );
