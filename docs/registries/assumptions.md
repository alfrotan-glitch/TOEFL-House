...
| # | Statement | Class | Affects | Basis / why unresolved |
|---|---|---|---|---|
| A-11 | The system needs no automated database backup mechanism | **REFUTED — DECISION** (owner, 2026-08-21) | Operations, §50 | Automated backups are required for the single-PC Windows deployment: take one on startup when no current backup exists and every 24 hours while running; retain 7 daily, 4 weekly and 12 monthly snapshots; every successful run requires a verified local SQLite snapshot and a verified external/network copy. See D-62. Implementation evidence is required before operational readiness is certified |
