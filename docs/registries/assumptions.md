...
| A-18 | The enrolment auto-invoice bills `other` rather than `tuition` | **RESOLVED — owner decided (D-136/D-137/D-138), implemented** | WP-07, WP-05 | `catalog-service.ts:381-386` puts a `registration` fee and a `semester` fee on ONE document, so it bills a mixture and can name no single tuition obligation — which D-118 requires of a tuition invoice. `invoices.purpose` is NOT NULL, so a value had to be written. `other` is strictly better than the `fee` it replaces (which paid down OTHER terms' debt) and forfeits nothing, because `enrollment-service.ts:162` inserts that path's `student_semesters` row with `fee_amount = 0` — the term bills nothing of its own. Resolving it properly means either splitting the enrolment invoice per purpose or writing the tuition figure onto the semester row, both of which change WP-05-certified writers. **Resolved 2026-08-21.** The owner chose the split model on the read-only analysis (`docs/work-packages/WP-07-F18-enrolment-invoice-analysis.md`): the term now carries its tuition, a `tuition` invoice names that term's obligation and an `other` invoice carries the fees alongside it. One residual is recorded in D-139: an enrolment that creates no term at all bills its whole snapshot as one `other` document, because there is no obligation for a tuition invoice to name |

## WP-12 — Workflow & Automation Assumptions

- Until an owner decision says otherwise, automation actions are intentionally
  limited to internal notifications; no generic create/update/webhook/
  transition side effect is assumed safe to execute.
